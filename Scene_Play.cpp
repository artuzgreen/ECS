...
void Scene_Play::sMovement()
{
	...
	{
		if (patrol->getComponent<CTransform>().pos != patrol->getComponent<CPatrol>().vertex[patrol->getComponent<CPatrol>().currentPos])
		{
			continue;
		}
...
void Scene_Play::sCollision()
{
	...
	for (auto &patrol : m_entityManager.getEntities("patrol_cutter"))
	{
		if (!patrol->hasComponent<CBoundingBox>())
		{
			continue;
		}
		...
		if (!m_player->hasComponent<CBoundingBox>() ||
			...
			patrol->hasComponent<CInvincible>())
		{
			continue;
		}
	...
	for (auto &bullet : bullets)
	{
		...
		for (auto &npc : npcs)
		{
			if (npc->hasComponent<CInvincible>())
			{
				continue;
			}
...
void Scene_Play::sDoAction(const Action &action)
{
	if (action.type() == "START")
	{
		...
		else if (action.name() == "SAVE_LEVEL")
		{
			...
			sl << sls.str();
			std::cout << "Level saved to "
					  << "res/save_level/saved_level.txt" << std::endl;
			sl.close();
		}
	}
